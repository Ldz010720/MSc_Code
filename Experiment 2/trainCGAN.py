...
import cfg
from DataLoader_custom import mitbih_train_custom
...
def main_worker(gpu, ngpus_per_node, args):
    ...
    gen_net = Generator(seq_len=280, channels=1, num_classes=1, latent_dim=200, data_embed_dim=10,
    ...
    #                         n_classes=5)
    dis_net = Discriminator(in_channels=1, patch_size=1, data_emb_size=50, label_emb_size=10, seq_length=280, depth=3,
                            n_classes=1)
    print(type(dis_net))
    ...
    # load dataset
    train_set = mitbih_train_custom(args=args)
    ...
    if args.load_path:
        ...
        # create new log dir
        print('create new log dir')
        assert args.exp_name
        args.exp_name = f"{args.exp_name}_channel{args.gen_channel}"
        # if args.rank == 0:
    ...
    for epoch in range(int(start_epoch), int(args.max_epoch)):
        ...
        #         'path_helper': args.path_helper,
        #         'fixed_z': fixed_z
        #     }, is_best, args.path_helper['ckpt_path'], filename=f"checkpoint{epoch + 1}")
...
def gen_plot(gen_net, epoch):
    ...
    for i in range(10):
        fake_noise = torch.FloatTensor(np.random.normal(0, 1, (1, 200)))
