...
class Generator(nn.Module):
    def __init__(self, seq_len=150, channels=3, num_classes=9, latent_dim=100, data_embed_dim=10,
        ...
        #     nn.Conv1d(self.data_embed_dim, self.channels, kernel_size=3, stride=1, padding=1),
        #     nn.Conv1d(self.data_embed_dim, self.channels, kernel_size=1, stride=1, padding=0),
        # )

    def forward(self, z, labels):
        c = self.label_embedding(labels)
        x = torch.cat([z, c], 1)  # d 维度拼接
        x = self.l1(x)
        x = x.view(-1, self.seq_len, self.data_embed_dim)  # b,seq_len,d
        H, W = 1, self.seq_len
        x = self.blocks(x)
        x = x.reshape(x.shape[0], 1, x.shape[1], x.shape[2])  # b,1,seq_len,d

        output = self.deconv(x.permute(0, 3, 1, 2))  # b,d,1,token -> b,channels,1,seq_len

        # x = x.permute(0, 3, 1, 2)  # b,d,1,token
        # x = x.reshape(x.shape[0], x.shape[1], x.shape[3])
        # output = self.deconv2(x)
        # output = output.reshape(x.shape[0], x.shape[1], 1, x.shape[2])
