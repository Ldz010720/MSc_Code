...
for j in range(repeat_freq):
    ...
    ground_truth_all.append(normalize(gts))
...
# # fig.suptitle(f"Comparison of Generated ECG Signal and Ground truth")  # 大标题
...
axs[0].plot(x, y)
axs[0].set_xlim(x[0] - 0.1, x[-1] + 0.1)

axs[0].set_title('a', fontsize=20)
axs[0].set_xlabel('Time (s)', fontsize=15)
axs[0].set_ylabel('Normalize ECG Signal (Unitless)', fontsize=15)
axs[0].tick_params(axis='both', which='major', labelsize=13)  # 增大横纵坐标刻度字体

x = np.linspace(0, len(out_all_myself.flatten()) / hz, len(out_all_myself.flatten()))
y = ground_truth_all.flatten()
axs[1].plot(x, y)
axs[1].set_xlim(x[0] - 0.1, x[-1] + 0.1)

axs[1].set_title('b', fontsize=20)
axs[1].set_xlabel('Time (s)', fontsize=15)
axs[1].set_ylabel('Normalize ECG Signal (Unitless)', fontsize=15)
axs[1].tick_params(axis='both', which='major', labelsize=13)  # 增大横纵坐标刻度字体
