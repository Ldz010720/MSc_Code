...
def train(args, gen_net: nn.Module, dis_net: nn.Module, gen_optimizer, dis_optimizer, gen_avg_param, train_loader,
    ...
    for iter_idx, (real_imgs, real_img_labels) in enumerate(train_loader):
        global_steps = writer_dict['train_global_steps']

        # Adversarial ground truths
        real_imgs = real_imgs.type(torch.cuda.FloatTensor).cuda(args.gpu, non_blocking=True)  # b,d
        ...
        noise = torch.cuda.FloatTensor(np.random.normal(0, 1, (real_imgs.shape[0], args.latent_dim))).cuda(args.gpu,
                                                                                                           non_blocking=True)
        ...
        # print(fake_img_labels)
        # print(fake_img_labels.shape)
